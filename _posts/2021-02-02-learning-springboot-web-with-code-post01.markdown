...
- 매핑 테이블은 주로 '동사'나 '히스토리'를 의미하는 테이블을 의미
    - 예제에서 '회원이 영화에 대해서 평점을 준다'를 구성할 때 '평점을 준다'는 행위가 바로 매핑 테이블이 필요한 부분
    - '회원'이라는 주어와 '영화'라는 목적어가 있지만 이에 대한 '평점을 준다'는 부분이 중간에서 주어와 목적어를 연결하는 매핑 테이블이 담당하게 됨
        - @ManyToMany의 경우 관꼐를 설정할 수는 있지만 두 엔티티 간의 추가적인 데이터를 기록할 수는 없음
<br>
